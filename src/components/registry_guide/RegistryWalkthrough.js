...
class RegistryWalkthrough extends Component {
  ...
  render() {
    ...
    const { shouldRun, walkthroughSteps, shouldShowOverlay } = this.state;
  ...
  handleJoyrideCallback(result) {
    if (result.type === "overlay:click" || result.type === "finished") {
      ...
      result.step.name === "application-fourth-step" &&
      result.action === "next" &&
      result.type === "step:before"
    ) {
      ...
      result.step.name === "application-fifth-step" &&
      result.type === "step:after"
    ) {
      ...
      result.step.name === "application-sixth-step" &&
      result.action === "back" &&
      result.type === "step:after"
    ) {
      this.pauseJoyride();
      ...
      result.step.name === "challenge-first-step" &&
      result.action === "next"
    ) {
      // this.setState({
      //   staticContainer: 'applicationDomains',
      ...
      result.step.name === "challenge-second-step" &&
      result.type === "step:after"
    ) {
      if (result.action === "next") {
        this.pauseJoyride();
      ...
      result.step.name === "challenge-third-step" &&
      result.type === "step:after"
    ) {
      if (result.action === "next") {
        this.pauseJoyride();
      ...
      result.step.name === "challenge-fourth-step" &&
      result.action === "back" &&
      result.type === "step:after"
    ) {
      this.pauseJoyride();
      ...
      result.step.name === "vote-first-step" &&
      result.action === "next"
    ) {
      // this.setState({
      ...
      result.step.name === "vote-second-step" &&
      result.type === "step:after"
    ) {
      if (result.action === "next") {
        this.pauseJoyride();
      ...
      result.step.name === "vote-third-step" &&
      result.action === "back" &&
      result.type === "step:after"
    ) {
      this.pauseJoyride();
      ...
      result.step.name === "reveal-first-step" &&
      result.action === "next"
    ) {
      // this.setState({
      ...
      result.step.name === "reveal-second-step" &&
      result.type === "step:after"
    ) {
      if (result.action === "next") {
        this.pauseJoyride();
      ...
      result.step.name === "reveal-third-step" &&
      result.action === "back" &&
      result.type === "step:after"
    ) {
      this.pauseJoyride();
      ...
      result.step.name === "domainsjourney-first-step" &&
      result.action === "next" &&
      result.type === "step:after"
    ) {
      this.pauseJoyride();
      ...
      result.step.name === "domainsjourney-second-step" &&
      result.type === "step:after"
    ) {
      if (result.action === "next") {
        this.pauseJoyride();
      ...
      result.step.name === "domainsjourney-third-step" &&
      result.type === "step:after"
    ) {
      if (result.action === "next") {
        this.pauseJoyride();
      ...
      result.step.name === "domainsjourney-fourth-step" &&
      result.type === "step:after"
    ) {
      if (result.action === "next") {
        this.pauseJoyride();
      ...
      result.step.name === "domainsjourney-fifth-step" &&
      result.type === "step:after"
    ) {
      if (result.action === "next") {
        this.pauseJoyride();
      ...
      result.step.name === "domainsjourney-sixth-step" &&
      result.type === "step:after" &&
      result.action === "back"
    ) {
      this.pauseJoyride();
  ...
  startJoyride(topic, steps) {
    let staticContainer;
    if (
      steps[0].name.includes("application") ||
      steps[0].name.includes("challenge")
    ) {
      staticContainer = "applicationDomains";
  ...
  updateDomainJourney(domainJourney) {
    PubSub.publish(
      "RegistryGuideStaticDomainJourney.updateDomainJourney",
      domainJourney,
    );
  }
  ...
  pubsubSubscription() {
    this.startWalkthroughEvent = PubSub.subscribe(
      "RegistryWalkthrough.startJoyride",
      this.startJoyride,
    );
    this.handleWalkthroughCallbackEvent = PubSub.subscribe(
      "RegistryWalkthrough.handleJoyride",
      this.handleJoyrideCallback,
    );
    this.resumeWalkthroughEvent = PubSub.subscribe(
      "RegistryWalkthrough.resumeJoyride",
      this.resumeJoyride,
    );
    this.toggleOverlayEvent = PubSub.subscribe(
      "RegistryWalkthrough.toggleOverlay",
      this.toggleOverlay,
    );
    this.confirmWalkthroughEvent = PubSub.subscribe(
      "RegistryWalkthrough.confirmWalkthrough",
      this.confirmWalkthrough,
    );
    this.resetStepsEvent = PubSub.subscribe(
      "RegistryWalkthrough.resetSteps",
      this.resetSteps,
    );
  }
}
